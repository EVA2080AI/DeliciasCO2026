...
import { isThumbPath, publicUrlForPath } from '@/lib/imageUrls';
import { formatBytes, isImagePath, listAllObjects } from '@/lib/mediaOptimizer';
...
type Item = { path: string; url: string; size: number };
      ...
      try {
        ...
        setFiles(
          objects
            ...
            .map((o) => ({ path: o.path, url: publicUrlForPath(o.path), size: o.size })),
        );
      } catch (err) {
    ...
    <AnimatePresence>
      {isOpen && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Seleccionar imagen de la librería"
        >
          <motion.div
          ...
          >
            <div className="flex items-center justify-between p-4 border-b">
              ...
              <button onClick={onClose} className="p-2 rounded-xl hover:bg-secondary text-muted-foreground transition-colors" aria-label="Cerrar">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {loading ? (
                ...
                <div className="text-center py-20 text-destructive text-sm">No se pudo cargar la galería: {error}</div>
              ) : files.length === 0 ? (
