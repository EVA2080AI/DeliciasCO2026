import { useEffect } from 'react';
import { useSiteSettingsMap } from '@/hooks/useSiteSettings';

/**
 * Convierte un color hex (#rrggbb) a componentes HSL listos para CSS vars.
 * Las custom properties de Tailwind esperan "H S% L%" sin hsl().
 */
function hexToHSLComponents(hex: string): string | null {
  const clean = hex.replace('#', '');
  if (clean.length !== 6) return null;
  const r = parseInt(clean.slice(0, 2), 16) / 255;
  const g = parseInt(clean.slice(2, 4), 16) / 255;
  const b = parseInt(clean.slice(4, 6), 16) / 255;
  const max = Math.max(r, g, b);
  const min = Math.min(r, g, b);
  let h = 0, s = 0;
  const l = (max + min) / 2;
  if (max !== min) {
    const d = max - min;
    s = l > 0.5 ? d / (2 - max - min) : d / (max + min);
    switch (max) {
      case r: h = ((g - b) / d + (g < b ? 6 : 0)) / 6; break;
      case g: h = ((b - r) / d + 2) / 6; break;
      case b: h = ((r - g) / d + 4) / 6; break;
    }
  }
  return `${Math.round(h * 360)} ${Math.round(s * 100)}% ${Math.round(l * 100)}%`;
}

const DynamicTheme = () => {
  const { settings, isLoading } = useSiteSettingsMap();

  useEffect(() => {
    if (isLoading) return;

    const applyTheme = () => {
      const root = document.documentElement;
      const isDark = root.classList.contains('dark');

      const applyColor = (cssVars: string[], rawValue: string, darkBoost = false) => {
        let hsl = rawValue.trim();
        if (hsl.startsWith('#')) {
          const converted = hexToHSLComponents(hsl);
          if (!converted) return;
          if (isDark && darkBoost) {
            const parts = converted.split(' ');
            const lValue = parseFloat(parts[2]);
            const boosted = Math.min(lValue + 12, 75);
            hsl = `${parts[0]} ${parts[1]} ${boosted}%`;
          } else {
            hsl = converted;
          }
        }
        cssVars.forEach(cssVar => root.style.setProperty(cssVar, hsl));
      };

      if (settings.brand_color_primary) {
        applyColor(['--primary', '--accent', '--ring'], settings.brand_color_primary, true);
        if (!isDark) {
          applyColor(['--terracotta', '--section-terracotta'], settings.brand_color_primary);
        }
      }

      const display = settings.brand_font_display || 'Plus Jakarta Sans';
      const body = settings.brand_font_body || 'Plus Jakarta Sans';
      root.style.setProperty('--font-display', `'${display}', sans-serif`);
      root.style.setProperty('--font-body', `'${body}', sans-serif`);

      const fonts = new Set([display, body]);
      fonts.forEach(font => {
        if (font === 'Plus Jakarta Sans') return;
        const id = `gfont-${font.replace(/\s+/g, '-')}`;
        if (!document.getElementById(id)) {
          const link = document.createElement('link');
          link.id = id;
          link.rel = 'stylesheet';
          link.href = `https://fonts.googleapis.com/css2?family=${encodeURIComponent(font)}:wght@300;400;500;600;700;800&display=swap`;
          document.head.appendChild(link);
        }
      });
    };

    // Apply immediately
    applyTheme();

    // Re-apply whenever the `.dark` class is toggled (ThemeToggle)
    const observer = new MutationObserver(applyTheme);
    observer.observe(document.documentElement, { attributes: true, attributeFilter: ['class'] });

    return () => observer.disconnect();
  }, [settings, isLoading]);

  return null;
};

export default DynamicTheme;
